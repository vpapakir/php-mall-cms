<?=boxHeader(array('title'=>'ManagePropertyComments.property.title'))?>
	<? $categoryID = input('CategoryID');  $sectionID = input('SectionID'); ?>
		...
		<form name="getProperties" method="post">
			...
			<?
				$options[0]['id'] = '';
				$options[0]['value'] = lang('SelectPropertyCategoriesForList.property.tip');
				//print_r($out['DB']['PropertyCategories']);
				//print_r($options);
				echo getLists($out['DB']['PropertyCategories'],$categoryID,array('name'=>'CategoryID','id'=>'id','value'=>'value','action'=>'submit();','options'=>$options))
			?>	
			<?
				$options[0]['id'] = '';
				$options[0]['value'] = lang('SelectPropertyTypeForList.property.tip');
				echo getLists($out['DB']['PropertyTypes'],$propertyType,array('name'=>'PropertyType','id'=>'PropertyTypeAlias','value'=>'PropertyTypeName','action'=>'submit();','options'=>$options));	
			?>	
			<?
				$options[0]['id'] = '';
