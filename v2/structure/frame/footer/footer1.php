...
<?php
            try
            {
                ...
                if(($data = $query->fetch()) != false)
                {
                    $structure_footer_title = $data['L'.$main_id_language];
                }
                $query->closeCursor();
                #intro
                $prepared_query = 'SELECT * FROM page
                ...
                if(($data = $query->fetch()) != false)
                {
                    $structure_footer_intro = $data['L'.$main_id_language];
                }
                $query->closeCursor();
                #desc
                $prepared_query = 'SELECT * FROM page
                ...
                if(($data = $query->fetch()) != false)
                {
                    $structure_footer_desc = $data['L'.$main_id_language];
                }
                $query->closeCursor();
                $query->execute();
            }
            ...
            if(!empty($structure_footer_title))
            {
?>
                <tr>
                    <td>
                        <h4 class="font_main"><?php echo($structure_footer_title); ?></h4>
                    </td>
                </tr>
<?php
            }
                
            if(!empty($structure_footer_intro))
            {
?>
                <tr>
                    <td>
                        <h4 class="font_main"><?php echo($structure_footer_intro); ?></h4>
                    </td>
                </tr>
<?php
            }
            
            if(!empty($structure_footer_desc))
            {
?>
                <tr>
                    <td>
                        <h4 class="font_main"><?php echo($structure_footer_desc); ?></h4>
                    </td>
                </tr>
<?php
            }
...
<?php
#redirection to Sale Search when user clicked quicksearch button
if(isset($_POST['bt_box_quicksearch']))
{
    if($_SESSION['index'] == 'index.php')
    {
        header('Location: '.$config_customheader.'ventes');
    }
    else
    {
