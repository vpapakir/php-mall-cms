    ...
    {       
        ...
        else
        {
			if($_SESSION['current_log_rightsuser'] < 7) {
				// TODO: If not admin user, fallback to default translation
				try {
				$prepared_query = 'SELECT id_translation, L1 FROM translation
                           WHERE code_translation = :code';
				//if((checkrights($main_rights_log, '9', $redirection)) === true){ $_SESSION['prepared_query'] = $prepared_query; }
				$query = $connectData->prepare($prepared_query);
				$query->bindParam('code', trim(htmlspecialchars("FR", ENT_QUOTES)));
				$query->execute();
