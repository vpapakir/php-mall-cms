...
if(isset($_POST['nbroomsout']))
{
    if(!empty($_POST['nbroomsout']))
    { 
        $total_piecesoutexterior = trim(htmlspecialchars($_POST['nbroomsout'], ENT_QUOTES)); 
    }
    else
    {
        $total_piecesoutexterior = 0;
    }
    ...
    while($data = $query->fetch())
    {
        ...
        $kprodimmo_nameS_piecesoutexterior[$i] = $data['L'.$_SESSION['current_language'].'S'];
        $kprodimmo_nameP_piecesoutexterior[$i] = $data['L'.$_SESSION['current_language'].'P'];
        $i++;
    }
    $query->closeCursor();
    ...
    while($data = $query->fetch())
    {
        ...
        $kprodimmo_nameS_piecesoutdetailsexterior[$i] = $data['L'.$_SESSION['current_language'].'S'];
        $kprodimmo_nameP_piecesoutdetailsexterior[$i] = $data['L'.$_SESSION['current_language'].'P'];
        $i++;
    }
    $query->closeCursor();
    ...
    function cdreditor($type, $valueP, $code, $status, $id, $selected, $isfirstoption, $insertinput, $inputstyle, $inputtype, $inputname, $inputother, $inputsession, $quicksearch, $chknrtd, $displaydecimal, $showresult, $datacolumn, $dataoperator, $cutstring, $cutcount, $cutpunctuation, $js)
    {
        ...
        {
            $header = $COOBOX_BASE_URL;
            $include_dbconnect_info = 'modules/dbconnect/dinx2.0/dbconnect_info.php';
            $main_customfolder = 'immo';
            include('modules/dbconnect/dinx2.0/dbconnect.php');
        }
    ...
    if(!empty($_SESSION['Kprodimmo_exterior_cdreditor_piecesout_exterior']))
    {
    ...
    for($i = 1, $y = 0; $i <= $total_piecesoutexterior; $i++, $y++)
    {
        #exterior pieces
