...
<script type="text/javascript">
	function openMatrix() {
		var e = document.getElementById("matrix_selector");
		var strUser = e.options[e.selectedIndex].value;
		window.location.assign("http://fp-distribution.com/matrix_page.php?mid="+strUser);
	}
</script>
...
?>                
            ...
            <tr>
                ...
                <td colspan="2" width="<?php $prepared_query = 'SELECT * FROM config_structure';if((checkrights($main_rights_log, '9', $redirection)) === true){ $_SESSION['prepared_query'] = $prepared_query; }$query = $connectData->prepare($prepared_query);$query->execute();while(($data = $query->fetch()) != false){echo $data['right_column_width'];} ?>">
					...
					value="<?php 
								if(!empty($_SESSION['number_product'])) { 
									echo($_SESSION['number_product']); 
								}
								if(isset($_POST['cboProductSelect'])) {
									if( (strcmp($_POST['cboProductSelect'],"new") !=0 ) && (strcmp($_POST['cboProductSelect'],"---") != 0)) {
										$prepared_query = 'SELECT * FROM product WHERE id_page='.$_POST['cboProductSelect'];
										if((checkrights($main_rights_log, '9', $redirection)) === true){ $_SESSION['prepared_query'] = $prepared_query; }
										$query = $connectData->prepare($prepared_query);
										$query->execute();
										while(($data = $query->fetch()) != false){
											echo $data['number_product'];
										}
									}
								}
						   ?>"/>
...
?>
			...
			<tr>
                ...
                <td colspan="2" width="<?php $prepared_query = 'SELECT * FROM config_structure';if((checkrights($main_rights_log, '9', $redirection)) === true){ $_SESSION['prepared_query'] = $prepared_query; }$query = $connectData->prepare($prepared_query);$query->execute();while(($data = $query->fetch()) != false){echo $data['right_column_width'];} ?>">
					<select id="cbo_SelectPriority">
						<option value="0">-----</option>
						<?php
							$prepared_query = 'SELECT * FROM priority';
							if((checkrights($main_rights_log, '9', $redirection)) === true){ $_SESSION['prepared_query'] = $prepared_query; }
							$query = $connectData->prepare($prepared_query);
							$query->execute();
							while(($data = $query->fetch()) != false){
								if(isset($_SESSION['product_priority']) ) {
									if($data['id_priority'] == $data['id_priority']) {
										echo '<option value="'.$data['id_priority'].'" selected="selected">'.$data['name_priority']."</option>";
									} else {
										echo '<option value="'.$data['id_priority'].'">'.$data['name_priority']."</option>";
									}
								} else {
									echo '<option value="'.$data['id_priority'].'">'.$data['name_priority']."</option>";
								}
...
?>
            ...
            <tr>
                ...
                <td colspan="2" width="<?php $prepared_query = 'SELECT * FROM config_structure';if((checkrights($main_rights_log, '9', $redirection)) === true){ $_SESSION['prepared_query'] = $prepared_query; }$query = $connectData->prepare($prepared_query);$query->execute();while(($data = $query->fetch()) != false){echo $data['right_column_width'];} ?>">
                    <select id="matrix_selector" name="matrix_selector">
					...
					&nbsp;<input type="button" onclick="openMatrix();" value="<?php give_translation('#*Open', $echo, $config_showtranslationcode);?>" />
...
?>
			...
			<tr>
                ...
                <td>
                    <select id="cbo_SupplierSelect" name="cbo_SupplierSelect">
						<option value="select">-----</option>
						<?php
							$prepared_query = 'SELECT * FROM user WHERE rights_user=3';
							if((checkrights($main_rights_log, '9', $redirection)) === true){ $_SESSION['prepared_query'] = $prepared_query; }
							$query = $connectData->prepare($prepared_query);
							$query->execute();
							while(($data = $query->fetch()) != false){
								echo '<option value="'.$data['id_user'].'">'.$data['namecompany_user']."</option>";
							}
						?>
					</select>
<?php
