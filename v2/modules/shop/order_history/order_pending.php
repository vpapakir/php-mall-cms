...
?>
                <td <?php echo($style_order_history_pending); ?> align="center">
                    <span id="center_text"><?php 
					if($data['status_order_history'] == 'paid') {
						try {
							$sql = "INSERT INTO tracking (id_tracking, id_order, number_tracking) VALUES ('".htmlspecialchars($data['id_order_history'], ENT_QUOTES, 'UTF-8')."', '".htmlspecialchars($data['id_order_history'], ENT_QUOTES, 'UTF-8')."','9999') ON DUPLICATE KEY UPDATE id_order=values(id_order), number_tracking=values(number_tracking) ";
							$prepared_query_tnumber = $sql;
							$query_tnumber = $connectData->prepare($prepared_query_tnumber);
							$query_tnumber->execute();
							$query_tnumber->closeCursor();
						} catch(Exception $e) {
							die('<br>Error: '.$e->getMessage());
						}
						echo '<span id="other_link" style="cursor: pointer;color:blue;text-align:center;font-size:12px;" onclick="popup2(\'shop/order_history/tracking_number_form.php?ordid='.$data['id_order_history'].'\', \'256\', \'256\')">'.$data['number_order_history'].'</span>'; 
					} else { 
						echo($data['number_order_history']); 
					} 
					?></span>
                </td>
