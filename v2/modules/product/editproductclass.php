...
//if( isset($_POST['shopsForProductGroup']) ) {
	...
	$prepared_query = "UPDATE product_class SET status_class_product='".$_POST['productClassStatusEdit']."',code_class_product=".rand().",";
	
	for($counter = 0; $counter < $i; $counter++) {
		$prepared_query = $prepared_query."name_class_product_L".($counter+1)."='".$_POST['editProductClassName'.$codes[$counter]]."',";
	}
	
	$prepared_query = $prepared_query."product_class_id='";
	foreach ($_POST['typesForProductClass'] as $shops)
	{
        $prepared_query = $prepared_query.$shops.",";
	}
	$prepared_query = $prepared_query."' WHERE name_product_class_L1='".$_POST['editProductClassNameFR']."'";
