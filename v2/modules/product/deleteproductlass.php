...
if( isset($_POST['editProductClassNameFR']) ) {
	$productClassNameEdit = $_POST['editProductClassNameFR'];
	$prepared_query = 'DELETE FROM product_class WHERE name_class_product_L1=:name';
	
	if((checkrights($main_rights_log, '9', $redirection)) === true){ $_SESSION['prepared_query'] = $prepared_query; }
    $query = $connectData->prepare($prepared_query);
    $query->execute(array(
                      'name' => $productClassNameEdit
                      ));
	$query->closeCursor();
	$result = 0;
	echo $result;
