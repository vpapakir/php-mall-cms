<?php
    unset($id_box_content, $code_box_content, $type_box_content, $typelink_box_content,
          ...
          $position_box_content, $loggedbox_msg_unread_count);
...
//{
    ...
    {  
        ...
        {
            $loggedbox_msg_unread_count = $data[0];
        }
        $query->closeCursor();
        
        #COUNT USER PORTFOLIO
        $prepared_query = 'SELECT COUNT(id_portfolio) FROM immo_portfolio
        ...
        {
            $countportfolio_box_content = $data[0];
        }
        $query->closeCursor();
...
<?php
        ...
        { 
            ...
            $bok_display_box_content = true;
            if($countportfolio_box_content > 0)
            {
                $title_box_content[$x] = str_replace('[#nr_portfolio]', '('.$countportfolio_box_content.')', $title_box_content[$x]);               
            }
            else
            {
                if(preg_match('#\[\#nr_portfolio\]#', $title_box_content[$x]))
                {
                    $bok_display_box_content = false;
                }
            }
            
            if($bok_display_box_content === true)
            {
...
?>
                        <td width="65%" align="left">                    
                            <a class="<?php echo($type_box_content[$x]); ?>" style="margin: <?php echo($result_margin_box_content); ?>;" href="<?php echo($href_box_content); ?>"><?php echo($title_box_content[$x]); ?>
<?php
                            if($data['url_page'] == 'myaccount' && $loggedbox_msg_unread_count > 0)
                            {
?>
                                <img src="<?php echo($config_customheader); ?>graphics/icons/mail/mailunread.gif" alt="mailunread.gif" style="border: none;"/>
<?php
                            }
?>
                            </a>
