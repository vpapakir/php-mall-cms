<?php
if(isset($_POST['bt_edit_district_geo']))
{
    ...
    if($Bok_cdrgeo_update === true)
    {
        try
        {
            ...
            {
                $Bok_cdrgeo_imageupdate_district = 'true';
                $cdrgeo_idimage_district = $data[0];
            }
            else
            {
                $query->closeCursor();
                if(!empty($upload_district))
                {
                    ...
                    $query->closeCursor();
                    $cdrgeo_idimage_district++;
                }
                else
                {
                    $cdrgeo_idimage_district = 0;
                }
                $Bok_cdrgeo_imageupdate_district = 'false';
        ...
        {   
            ...
            upload_file('upload_cdrgeo_district',
                        ...
                        null,
                        'false',
                        $Bok_cdrgeo_imageupdate_district,
                        $cdrgeo_idimage_district);
        }
        ...
        {           
            $prepared_query = 'UPDATE cdrgeo
                               SET type_cdrgeo = :type,
                                   ...
                                   parentdepartment_cdrgeo = :parent,
                                   pageinfo_cdrgeo = :pageinfo,
                                   id_image = :image, ';
            ...
            for($i = 0, $count = count($main_activatedidlang); $i < $count; $i++)
            {
                if($i == ($count - 1))
                {
                    $prepared_query .= 'L'.$main_activatedidlang[$i].' = "'.$cdrgeo_name_district[$i].'"';
                }
                else
                {
                    $prepared_query .= 'L'.$main_activatedidlang[$i].' = "'.$cdrgeo_name_district[$i].'", ';
            ...
            $query->execute(array(
                                  ...
                                  'image' => $cdrgeo_idimage_district,
                                  'id' => $cdrgeo_id_district
                                  )); 
            $query->closeCursor();           
            ...
            $_SESSION['msg_cdrgeo_done'] = 'L\'arrondissement "'.$cdrgeo_name_district[$cdrgeo_selected_lang].'" a été modifié';
        }
        catch(Exception $e)
        {
            $_SESSION['error400_message'] = $e->getMessage();
