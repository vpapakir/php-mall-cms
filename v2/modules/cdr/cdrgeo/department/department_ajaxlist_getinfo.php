...
if(!empty($_POST['SearchCDRgeoDepartmentValue']))
{
    #department
    unset($_SESSION['cdrgeo_edit_button_department'],
            $_SESSION['cdrgeo_hiddenidCDRgeoDepartment'],
            $_SESSION['cdrgeo_txtSearchCDRgeoDepartment']);

    for($i = 0, $count = count($id_language); $i < $count; $i++)
    {
       unset($_SESSION['cdrgeo_txtNameCDRgeoDepartment'.$id_language[$i]]); 
    }
    ...
    if($_POST['SearchCDRgeoDepartmentValue'] != 'new')
    {
        ...
        $prepared_query = 'SELECT *
                           ...
                           WHERE id_cdrgeo = :id';
        $query = $connectData->prepare($prepared_query);
        $query->bindParam('id', $_POST['SearchCDRgeoDepartmentValue']);
        $query->execute();
        ...
        if(($data = $query->fetch()) != false)
        {
            ...
            for($i = 0, $count = count($id_language); $i < $count; $i++)
            {
               $_SESSION['cdrgeo_txtNameCDRgeoDepartment'.$id_language[$i]] = $data['L'.$id_language[$i]]; 
            }
