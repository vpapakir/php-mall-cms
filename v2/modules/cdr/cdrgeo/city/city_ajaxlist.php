...
?>
<style>
.SearchCDRgeoCityResultList
{
   cursor: pointer; 
}    
    
.SearchCDRgeoCityResultList:hover
{
   background-color: lightblue; 
}
</style> 
<?php
if(!empty($_POST["txtSearchCDRgeoCity"]))
{
    ...
    $search_sentence = $_POST["txtSearchCDRgeoCity"];
    ...
    else
    {
        $prepared_query = 'SELECT *
                           ...
                           OR zip_cdrgeo LIKE "%'.$search_sentence.'%")
...
?>
    <li class="SearchCDRgeoCityResultList" style="cursor: pointer;" onclick="this.setAttribute('id', 'thisoption'); requestGEOEDITCityGetinfo(); reloadPage();">
        <span class="font_main" style="margin-left: 4px;">Nouveau</span>
        <input name="SearchCDRgeoCityResultListID" id="SearchCDRgeoCityResultListID<?php echo($i); ?>" style="display: none;" type="hidden" value="new"/>
    </li>
<?php
    ...
    if(($data = $query->fetch()) != false)
    {
        ...
        {
?>
            <li class="SearchCDRgeoCityResultList" <?php if(empty($data['statusobject_cdrgeo']) || $data['statusobject_cdrgeo'] == 9){ echo('style="background-color: lightblue;"'); } ?> onclick="this.setAttribute('id', 'thisoption'); requestGEOEDITCityGetinfo(); reloadPage();">
                ...
                <input name="SearchCDRgeoCityResultList" id="SearchCDRgeoCityResultList<?php echo($i); ?>" style="display: none;" type="hidden" value="<?php echo($data['L'.$_SESSION['current_language']]); ?>"/>
                <input name="SearchCDRgeoCityResultListID" id="SearchCDRgeoCityResultListID<?php echo($i); ?>" style="display: none;" type="hidden" value="<?php echo($data[0]); ?>"/>
                </span>
            </li>
<?php
            $i++;
