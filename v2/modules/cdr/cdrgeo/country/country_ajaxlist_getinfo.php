...
if(!empty($_POST['SearchCDRgeoCountryValue']))
{
    #country
    unset($_SESSION['cdrgeo_edit_button_country'],
            $_SESSION['cdrgeo_hiddenidCDRgeoCountry'],
            $_SESSION['cdrgeo_txtSearchCDRgeoCountry']);

    for($i = 0, $count = count($id_language); $i < $count; $i++)
    {
       unset($_SESSION['cdrgeo_txtNameCDRgeoCountry'.$id_language[$i]]); 
    }
    ...
    if($_POST['SearchCDRgeoCountryValue'] != 'new')
    {
        ...
        $prepared_query = 'SELECT *
                           ...
                           WHERE id_cdrgeo = :id';
        $query = $connectData->prepare($prepared_query);
        $query->bindParam('id', $_POST['SearchCDRgeoCountryValue']);
        $query->execute();
        ...
        if(($data = $query->fetch()) != false)
        {
            ...
            for($i = 0, $count = count($id_language); $i < $count; $i++)
            {
               $_SESSION['cdrgeo_txtNameCDRgeoCountry'.$id_language[$i]] = $data['L'.$id_language[$i]]; 
            }
...
//        $prepared_query = 'SELECT *
//                           FROM `cdrgeo`
//                           WHERE parentregion_cdrgeo = :id';
//        $query = $connectData->prepare($prepared_query);
//        $query->bindParam('id', $region_id);
//        $query->execute();
//
//        if(($data = $query->fetch()) != false)
//        {
...
//        $prepared_query = 'SELECT *
//                           FROM `cdrgeo`
//                           WHERE parentdepartment_cdrgeo = :id';
//        $query = $connectData->prepare($prepared_query);
//        $query->bindParam('id', $department_id);
//        $query->execute();
//
//        if(($data = $query->fetch()) != false)
//        {
...
//            for($i = 0, $count = count($id_language); $i < $count; $i++)
//            {
//               $_SESSION['cdrgeo_txtNameCDRgeoDistrict'.$id_language[0]] = $data['L'.$id_language[0]]; 
//            }
...
//        $prepared_query = 'SELECT *
//                           FROM `cdrgeo`
//                           WHERE parentdistrict_cdrgeo = :id';
//        $query = $connectData->prepare($prepared_query);
