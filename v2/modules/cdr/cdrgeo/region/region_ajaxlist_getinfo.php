...
if(!empty($_POST['SearchCDRgeoRegionValue']))
{
    #region
    unset($_SESSION['cdrgeo_edit_button_region'],
            $_SESSION['cdrgeo_hiddenidCDRgeoRegion'],
            $_SESSION['cdrgeo_txtSearchCDRgeoRegion']);

    for($i = 0, $count = count($id_language); $i < $count; $i++)
    {
       unset($_SESSION['cdrgeo_txtNameCDRgeoRegion'.$id_language[$i]]); 
    }
    ...
    if($_POST['SearchCDRgeoRegionValue'] != 'new')
    {
        ...
        $prepared_query = 'SELECT *
                           ...
                           WHERE id_cdrgeo = :id';
        $query = $connectData->prepare($prepared_query);
        $query->bindParam('id', $_POST['SearchCDRgeoRegionValue']);
        $query->execute();
        ...
        if(($data = $query->fetch()) != false)
        {
            ...
            for($i = 0, $count = count($id_language); $i < $count; $i++)
            {
               $_SESSION['cdrgeo_txtNameCDRgeoRegion'.$id_language[$i]] = $data['L'.$id_language[$i]]; 
            }
