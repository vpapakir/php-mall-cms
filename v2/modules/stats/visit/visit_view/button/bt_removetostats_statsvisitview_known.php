<?php
if(isset($_POST['bt_removetostats_statsvisitview_known']))
{
    ...
    {
        $stats_visitview_chk = htmlspecialchars($_POST['chk_statsvisit_known_'.$stats_visitview_known_idstatsvisit[$i]], ENT_QUOTES);
        ...
        if($stats_visitview_chk == 1)
        {
            ...
            try
            {
                ...
                $query->bindParam('idstatsonline', $stats_visitview_known_idstatsvisit[$i]);
                $query->execute();
                $query->closeCursor();            
            }
            catch(Exception $e)
