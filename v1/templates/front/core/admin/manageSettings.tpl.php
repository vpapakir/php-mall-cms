<?=boxHeader(array('title'=>'ManageSettings.core.title'))?>
	...
	<form name="manageSettings" method="post">
		...
		<tr> 
			<td valign="top" bgcolor="#ffffff" class="fieldNames">
					...
					<br/>
					<div align="center">
					<a href="<?=setting('url')?>manageSettingFront/GroupID/<?=input('GroupID')?>/Level2GroupID/<?=input('Level2GroupID')?>/" class="subtitle">[<?=lang('AddSetting.core.link')?>]</a>
					</div>		
					<br/>
					<? }*/ ?>
							...
							if(user('UserName') == 'superadmin'){?>
								...
								<tr>
									<td valign="top" class="row1" width="70%">
										...
										<? if(hasRights('root')) {?>
										<a href="<?=setting('url')?>manageSettingFront/SettingID/<?=$row['SettingID']?>/GroupID/<?=input('GroupID')?>/Level2GroupID/<?=input('Level2GroupID')?>">[<?=lang('-edit')?>]</a>&nbsp;&nbsp;<a href="<?=setting('url')?>settingsadminfront/Setting<?=DTR?>SettingID/<?=$row['SettingID']?>/actionMode/delete/GroupID/<?=input('GroupID')?>/Level2GroupID/<?=input('Level2GroupID')?>" onClick="return confirm('<?=lang('AreYouSureToDeleteSetting.core.tip')?>')">[<?=lang('-delete')?>]</a>
									...
									<td valign="top" class="row1">
										<? if($row['SettingValueType']=='file') { ?>			
											...
											<? } ?>			
												<br/>
												<a href="<?=setting('url')?>manageSettingFrontFile/SettingID/<?=$row['SettingID']?>/GroupID/<?=input('GroupID')?>/Level2GroupID/<?=input('Level2GroupID')?>"><?=lang('-uploadimage')?></a>
										...
										<? } elseif ($row['SettingValueType']=='color')  { ?>
											<?=getSettingsList('color','Setting'.DTR.'SettingValue_'.$row['SettingID'],$row['SettingValue'],array('code'=>'Y'));?>
										...
										<? } elseif($row['SettingValueType']=='text') { $settingID = $row['SettingID']; foreach($out['DB']['Languages']['languageCodes'] as $langID=>$langCode) { ?>
											...
											<br/>
											<a href="<?=setting('url')?>editSettingFrontText/SettingID/<?=$row['SettingID']?>/GroupID/<?=input('GroupID')?>/Level2GroupID/<?=input('Level2GroupID')?>"><?=lang('EditSettingText.core.link')?></a>
											<br/>
										<? } } elseif($row['SettingValueType']=='styletype'){ ?>
											...
											<br/>
											<a href="<?=setting('url')?>editSettingFrontStyle/SettingID/<?=$row['SettingID']?>/GroupID/<?=input('GroupID')?>/"><?=lang('EditWithStyle.core.link')?></a>
										<? }else{?>
	...
	?>
		<tr> 
			...
			<!-- <br/>
			<div align="center">
			<a href="<?=setting('url')?>manageSettingFront/GroupID/<?=input('GroupID')?>/Level2GroupID/<?=input('Level2GroupID')?>" class="boldLink">[<?=lang('AddSetting.core.link')?>]</a>
			</div>		
			<br/> -->			
			<br><br>
