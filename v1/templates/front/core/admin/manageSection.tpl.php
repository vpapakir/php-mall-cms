	...
	<form name="<?=$formName?>" method="post" onSubmit="submitonce(this)" enctype="multipart/form-data">
		...
		<tr> 
			<td valign="top" bgcolor="#ffffff" class="fieldNames">
					...
					&nbsp;&nbsp;<input type="button" value="<?=lang("SaveAndGoToWebsite.core.buttons")?>" onClick="document.manageSections.SID.value='adminPage';document.manageSections.afterSaveAction.value='goToWebsite';submit();">					
			...
			<td valign="top" bgcolor="#ffffff" class="fieldNames">
					<table cellspacing="0" cellpadding="4" width="100%">
						...
						<tr>
							...
							<td>
								<? if(empty($out['DB']['Section'][0]['SectionBoxStyle'])) $out['DB']['Section'][0]['SectionBoxStyle'] = 'defaultbox';?>
						...
						<tr>
							...
							<td>
								<? foreach($out['DB']['Languages']['languageCodes'] as $langID=>$langCode) { ?>
									...
									<?=getFormated(getValue($out['DB']['Section'][0]['SectionIntroContent'],$langCode),'HTML','form',array('fieldName'=>'Section'.DTR.'SectionIntroContent['.$langCode.']','editorName'=>'SectionIntroContent'.$langCode,'editorHeight'=>200,'editorWidth'=>500))?>
									<br/>
								<? } ?>								
							</td>
						</tr>						
						<tr>
							...
							<td>
								<? foreach($out['DB']['Languages']['languageCodes'] as $langID=>$langCode) { ?>
									<? if(count($out['DB']['Languages']['languageCodes']>1)) { ?><?=$out['DB']['Languages']['languageNames'][$langID]?><br/><? } ?>
									<?=getFormated(getValue($out['DB']['Section'][0]['SectionContent'],$langCode),'HTML','form',array('fieldName'=>'Section'.DTR.'SectionContent['.$langCode.']','editorName'=>'SectionContent'.$langCode,'editorWidth'=>500))?>
									<br/>
								<? } ?>								
							</td>
						</tr>
					...
					&nbsp;&nbsp;<input type="submit" value="<?=lang("-save")?>">
					&nbsp;&nbsp;<input type="button" value="<?=lang("SaveAndGoToWebsite.core.buttons")?>" onClick="document.manageSections.SID.value='adminPage';document.manageSections.afterSaveAction.value='goToWebsite';submit();">					
					<? if(!empty($nextView)) { ?>&nbsp;&nbsp;<input type="button" value="<?=lang("SaveAndGoToNextStep.core.button")?>" onClick="document.manageSections.SID.value='adminPage';document.manageSections.viewMode.value='<?=$nextView?>';submit();"><? } ?>
					&nbsp;&nbsp;<input type="button" value="<?=lang("-delete")?>" onClick="document.manageSections.actionMode.value='delete';confirmDelete('manageSections', '<?=lang("-deleteconfirmation")?>');">					
					<? } ?>
					&nbsp;&nbsp;<input type="button" value="<?=lang("-cancell")?>" onClick="document.manageSections.actionMode.value='cancell';submit();">
					<br/>
