				...
				<table border="0" cellspacing="1" cellpadding="5" width="100%">
					...
					<tr>
						...
						<td valign="top" class="row1" width="65%">
							...
							<a href="<?=setting('url')?>editOwnerSection/viewMode/main/SectionID/<?=$row['SectionID']?>/GroupID/<?=$sectionGroupID?>/tabLink/5/"><?=getValue($row['SectionName'])?></a>
							<? } else { ?>
						...
						<td valign="top" class="row1" width="10%" align="right">
							<select name="manageDD_<?=$id?>" onChange="selectLink('manageSections', 'manageDD_<?=$id?>', '<?=lang('AreYouSureToDeleteSection.core.tip')?>', '2')">
								<option value="0" selected><?=lang('SelectAction.core.tip')?></option>
								<option value="<?=setting('url')?>editOwnerSection/SectionID/<?=$row['SectionID']?>/GroupID/<?=$sectionGroupID?>/viewMode/main/tabLink/1/"><?=lang('-edit')?></option>
								<? /* option value="<?=setting('url')?>editOwnerSection/SectionID/<?=$row['SectionID']?>/GroupID/<?=$sectionGroupID?>/viewMode/advanced/tabLink/4/"><?=lang('SectionEditAdvanced.core.option')?></option */ ?>
								<option value="<?=setting('url')?><?=input('SID')?>/Section<?=DTR?>SectionID/<?=$row['SectionID']?>/SectionID/<?=$row['SectionParentID']?>/actionMode/delete/GroupID/<?=$sectionGroupID?>"><?=lang('-delete')?></option>
								<option value="<?=setting('url')?>editOwnerSection/SectionParentID/<?=$row['SectionParentID']?>/GroupID/<?=$sectionGroupID?>/RequestedGroupID/<?=$row['SectionGroupID']?>/SectionPosition/<? $newSectionPosition=$row['SectionPosition'] + 1; echo $newSectionPosition; ?>/viewMode/main/tabLink/1/"><?=lang('AddSectionAfter.core.link','nospace')?></option>
								<option value="<?=setting('url')?>editOwnerSection/SectionParentID/<?=$row['SectionID']?>/GroupID/<?=$sectionGroupID?>/RequestedGroupID/<?=$row['SectionGroupID']?>/SectionPosition/1/viewMode/main/tabLink/1/"><?=lang('AddSectionUnder.core.link','nospace')?></option>
							</select>
						</td>
					</tr>
					<?  } } else {  ?>	
					<tr>
						<td colspan="5" class="subtitle">
							...
							<a href="<?=setting('url')?>editOwnerSection/RequestedGroupID/<?=$sectionGroupCurrentID?>"><?=lang('AddSection.core.link')?></a>
							<? } else { ?>
									<div align="center">
										<a href="<?=setting('url')?>editOwnerSection/RequestedGroupID/<?=input('GroupID')?>" class="boldLink">[<?=lang('AddSection.core.link')?>]</a>
									</div>		
							<? } ?>
						</td>
					</tr>
