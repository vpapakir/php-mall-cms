<?=boxHeader(array('title'=>lang('ResourceOrder.resource.title')))?>
	<tr>
		<form name="getOrders" method="post">
			...
			<?
				$options[0]['id'] = '';
				$options[0]['value'] = lang('AllPaymentStatuses.resource.tip');
				echo getReference('ResourceOrderPaymentStatus','ResourceOrder'.DTR.'ResourceOrderPaymentStatus',input('ResourceOrder'.DTR.'ResourceOrderPaymentStatus'),array('code'=>'Y','type'=>'dropdown','action'=>'submit();','options'=>$options))
			?>	
			<?
				$options[0]['id'] = '';
				$options[0]['value'] = lang('AllOrderHandlingStatuses.resource.tip');
				echo getReference('ResourceOrderStatus','ResourceOrder'.DTR.'ResourceOrderStatus',input('ResourceOrder'.DTR.'ResourceOrderStatus'),array('code'=>'Y','type'=>'dropdown','action'=>'submit();','options'=>$options))
			?>	
