<?=boxHeader(array('title'=>lang('TourOrder.tour.title')))?>
	<tr>
		<form name="getOrders" method="post">
			...
			<?
				$options[0]['id'] = '';
				$options[0]['value'] = lang('AllPaymentStatuses.tour.tour.tip');
				echo getReference('TourOrderPaymentStatus','TourOrderPaymentStatus',input('TourOrderPaymentStatus'),array('code'=>'Y','type'=>'dropdown','action'=>'submit();','options'=>$options))
			?>	
			<?
				$options[0]['id'] = '';
				$options[0]['value'] = lang('AllOrderHandlingStatuses.tour.tip');
				echo getReference('TourOrderStatus','TourOrderStatus',input('TourOrderStatus'),array('code'=>'Y','type'=>'dropdown','action'=>'submit();','options'=>$options))
			?>	
