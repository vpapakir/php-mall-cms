<?=boxHeader(array('title'=>'ManageLastBlogRecords.blog.title'))?>
				...
				<table border="0" cellspacing="1" cellpadding="5" width="100%">
					...
					<tr>
						...
						<td valign="top" class="row1">
							<?=getValue($row['BlogRecordTitle'])?><!--  : <small><?=getFormated($row['TimeCreated'],'DateTime')?></small> -->
						</td>
						<td class="row1">
						...
						<td valign="top" class="row1" width="10%" align="right">
							...
							<select name="manageR<?=$row['BlogRecordID']?>" onChange="selectLink('manageBlogRecords', 'manageR<?=$row['BlogRecordID']?>', '<?=lang('AreYouSureToDeleteSection.core.tip')?>', '2')">
								...
								<option value="<?=setting('url')?>manageBlogRecords/BlogRecordID/<?=$row['BlogRecordID']?>/BlogID/<?=$row['BlogID']?>/CategoryID/<?=input('CategoryID')?>/SectionID/<?=input('SectionID')?>/BlogType/<?=input('BlogType')?>/actionMode/edit"><?=lang('-edit')?></option>
								<option value="<?=setting('url')?>homeadmin/BlogRecord<?=DTR?>BlogRecordID/<?=$row['BlogRecordID']?>/actionMode/delete/CategoryID/<?=input('CategoryID')?>/SectionID/<?=input('SectionID')?>/BlogType/<?=input('BlogType')?>"><?=lang('-delete')?></option>
							</select>
