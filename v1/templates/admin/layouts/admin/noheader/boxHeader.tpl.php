			...
			<td>
				...
				<? if(is_array($out['DB']['tabs'])) { $ind=1;
				 foreach($out['DB']['tabs'] as $row) { $selectedTab='';  if($ind==1) {$selectedTab=' class="selectedtab" '; $ind=2;} ?>
						...
						<? }  ?> 
				<? } } ?>
					<? if(input('windowMode')=='noheader') {  ?>
						<li><a href="<?=setting('rooturl').'go/'.setting('lang')?>/home/" target="_blank"><?=lang('GoToFront.core.link')?></a></li>
						<li><a href="<?=setting('rooturl').'adm/'.setting('lang')?>/" target="_blank"><?=lang('GoToAdmin.core.link')?></a></li>
						<li><a href="<?=setting('rooturl').'go/'.setting('lang')?>/logoutadminfront/actionMode/logout/"><?=lang('Logout.core.link')?></a></li>
					<? } ?>
